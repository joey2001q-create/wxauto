...
class WXWorkAutomation:
    ...
    def find_blue_add_button(self, screenshot_path=None):
        """使用OpenCV查找蓝色的"添加"按钮

        企业微信的"添加"按钮是蓝色背景白色文字
        通过颜色检测定位按钮位置
        ...
        logger.info(f"OpenCV找到蓝色'添加'按钮: ({center_x}, {center_y}), 大小{w}x{h}")

        return {
            "method": "opencv_blue_button",
            "window_pos": (center_x, center_y),
            "screen_pos": (rect.left + center_x, rect.top + center_y),
            "confidence": 0.9,
            "size": (w, h),
    ...
    def add_contact_by_phone(self, phone, verify_msg=None, timeout=10):
        ...
        # 调试：保存截图并输出OCR结果
        debug_screenshot = self.screenshot(save_path=f"debug_add_button_{phone}.png")
        all_items = self.ocr_scan()
        logger.info(f"调试 - 截图保存: {debug_screenshot}")
        logger.info(f"调试 - OCR识别到 {len(all_items)} 个文字:")
        for item in all_items:
            logger.info(f"  文字: '{item['text']}', 位置: {item['window_pos']}, 置信度: {item.get('confidence', 0):.2f}")

        # 方法1: OCR识别 + 位置筛选（右侧区域）
        items = all_items
        rect = self.get_window_rect()
        window_w = rect.right - rect.left
        # 修复：使用绝对阈值（左侧边栏约200px，右侧区域从200开始）
        right_threshold = max(200, window_w * 0.35)
        logger.info(f"调试 - 窗口宽度: {window_w}, 右侧阈值: {right_threshold}")

        for item in items:
            if "添加" in item["text"]:
                logger.info(f"调试 - 找到'添加': '{item['text']}' 位置: {item['window_pos']}")
                # 检查位置：在窗口右侧（x > 阈值）
                if item["window_pos"][0] > right_threshold:
                    add_item = item
                    logger.info(f"使用OCR找到'添加'按钮: 位置{item['window_pos']}")
                    break
                else:
                    logger.info(f"调试 - '添加'位置在左侧，跳过: {item['window_pos'][0]} < {right_threshold}")

        # 方法2: 基于"网络查找"结果位置推算（如果方法1失败）
        if not add_item and lookup_item:
            ...
            for item in items:
                if "添加" in item["text"]:
                    ...
                    # 添加按钮应在查找结果的右侧（x > lx + 50）且在同一水平线附近（|ay - ly| < 80）
                    if ax > lx + 50 and abs(ay - ly) < 80:
                        add_item = item
                        logger.info(f"使用相对位置找到'添加'按钮: 位置({ax}, {ay})，基于网络查找位置({lx}, {ly})")
                        break

        # 方法3: 如果还是找不到，放宽条件，只要找到"添加"就用（兜底）
        if not add_item:
            for item in items:
                if "添加" in item["text"]:
                    add_item = item
                    logger.info(f"使用兜底方案找到'添加'按钮: 位置{add_item['window_pos']}")
                    break
        ...
        # 注意：不再验证位置，因为"添加"按钮可能在"网络查找"的左侧或右侧
        # 取决于企业微信的界面布局
        ...
        tx, ty = popup_title["window_pos"]
        logger.info(f"弹窗标题位置: ({tx:.0f}, {ty:.0f})")

        # 筛选弹窗内的"发送"（在标题下方50-250px范围内）
        popup_send_items = []
        for item in send_items:
            ...
            if ty + 50 < y < ty + 250:
                ...
                logger.info(f"弹窗内'发送'候选: '{item['text']}' at ({x:.0f}, {y:.0f})")

        if popup_send_items:
            # 选择y最小的（最靠近标题的，排除标题本身）
            popup_send_items.sort(key=lambda x: x["window_pos"][1])
            send_btn = popup_send_items[0]
            logger.info(f"选择弹窗发送按钮: '{send_btn['text']}' at {send_btn['window_pos']}")
        elif send_items:
            # 兜底：使用原来的逻辑（y最大的，但要排除y < ty的）
            ...
            if valid_items:
                ...
                logger.warning(f"使用兜底方案选择发送按钮: '{send_btn['text']}' at {send_btn['window_pos']}")
            else:
                return {"status": "failed", "detail": "未找到有效的发送按钮"}
        else:
            return {"status": "failed", "detail": "未找到发送按钮"}

        bx, by = send_btn["screen_pos"]
        logger.info(f"点击'发送'按钮 at ({bx:.0f}, {by:.0f})")
